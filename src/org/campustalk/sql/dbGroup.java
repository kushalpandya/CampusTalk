...
public class dbGroup extends DatabaseManager {
	...
	public boolean AddGroup(String groupname, String description)
		...
		/**
		 * CREATE PROCEDURE `campustalk`.`insertGroup`( IN name VARCHAR(100), IN
		 * duration TEXT)
		 */
	...
	public boolean EditGroup(int groupid, String groupname, String description,
	...
	public void DeleteGroup(int id) throws SQLException, ClassNotFoundException {
		...
		this.open();
		CallableStatement csSql = CON.prepareCall("{call deleteGroupData(?)}");
		csSql.setInt(1, id);
	...
	public JSONArray getGroupData() {
		...
		try {
			this.open();
			CallableStatement csSql = CON.prepareCall("{call getGroupData()}");
	...
	/*
	 * public CampusTalkGroup[] getAllGroupData() throws SQLException,
	 * ClassNotFoundException {
	 * 
	 * this.open(); CallableStatement csSql = CON
	 * .prepareCall("{call getAllGroupData()}"); ResultSet rs =
	 * csSql.executeQuery(); ArrayList<CampusTalkGroup> Group = new
	 * ArrayList<>(); CampusTalkGroup temp_Group;
	 * 
	 * while(rs.next()) { temp_Group = new CampusTalkGroup();
	 ...
	 * 
	 * /*while(rs.next()) { objGroup[i].setName(rs.getString("name")); i++; }
	 * return objGroup; }
	 */
